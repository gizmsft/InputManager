...
; function InputManager(element, options) {
    ...
    var onCursorPositionChanged = function (e) {
    ...
    var onKeypress = function (e) {
        if (local.settings.events.onKeypress && local.enabled) {
            var state = getInputState();
            var param = $.extend(e, state);
            local.settings.events.onKeypress.call($self, param);
        }
    }
    ...
    $target.on('keypress keydown keyup input click paste change', onCursorPositionChanged);
}
